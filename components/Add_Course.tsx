...
import { addCourse, revalidatePage } from '@/server_actions';
...
export default function AddCourse() {
 ...
 const onSubmit = async (data: CourseFormType) => {
  ...
  const course = { id: Date.now().toLocaleString(), ...data };
  try {
 ...
 return (
  ...
  <Form {...form}  >
   ...
   <form onSubmit={form.handleSubmit(onSubmit)} className='p-4 md:flex items-center justify-between gap-3 w-full relative '>
    <FormField
     ...
     render={({ field }) => (
      <FormItem>
       ...
       <FormControl>
        <Input type='text' placeholder='course name' {...form.register('name')} className='pl-10' {...field} />
       </FormControl>
       <FormMessage />
      </FormItem>
     )}
    ...
    <FormField
     ...
     render={({ field }) => (
      <FormItem>
       ...
       <FormControl>
        <Input type='text' placeholder='category name' {...form.register('category')} className='pl-10' {...field} />
       </FormControl>
       <FormMessage />
      </FormItem>
     )}
    ...
    <FormField
     ...
     render={({ field }) => (
      <FormItem>
       ...
       <FormControl>
        <Input type='text' placeholder='course trainer' {...form.register('name')} className='pl-10' {...field} />
       </FormControl>
       <FormMessage />
      </FormItem>
     )}
